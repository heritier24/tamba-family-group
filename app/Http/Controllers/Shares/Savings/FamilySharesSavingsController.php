...
use App\Http\Requests\Savings\UpdateSavingTransactionRequest;
...
class FamilySharesSavingsController extends Controller
{
    ...
    public function updateSavingsTransactions(UpdateSavingTransactionRequest $request, int $transactionID)
    {
        try {
            $this->service->updateSavingsTransactions(
                $transactionID,
                $request->monthly_transaction,
                $request->amount_tobe_paid,
                $request->amount_paid,
                $request->remaining_amount,
                $request->new_upddated_amount,
                $request->transaction_to,
                $request->user_id
            );

            return response()->json(["Savings Transaction updated successfully"]);
        } catch (\Exception $e) {
            [$message, $statusCode, $exceptionCode] = getHttpMessageAndStatusCodeFromException($e);
